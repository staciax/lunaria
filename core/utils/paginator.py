...
class NumberedPageModal(discord.ui.Modal, title='Go to page'):
    ...
    async def on_submit(self, interaction: discord.Interaction[Lunaria]) -> None:
...
class FieldPageSource(ListPageSource):
    ...
    async def format_page(self, menu: LunaPages, entries: list[tuple[Any, Any]]) -> discord.Embed:
...
class SimplePages(LunaPages):
    """A simple pagination session reminiscent of the old Pages interface.

    Basically an embed with some normal formatting.
    """

    def __init__(self, entries, *, interaction: discord.Interaction[Lunaria], per_page: int = 12):
